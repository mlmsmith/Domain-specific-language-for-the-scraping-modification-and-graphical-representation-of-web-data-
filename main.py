...
class Main:
    """
    Runs the whole programme by invoking methods and instantiating classes in the appropriate order
    """
    def run(self):
        """
        ...
        except pandas.errors.EmptyDataError:  # check if data was successfully scraped
        ...
        if len(r.group_by) > 0:
            if r.aggregate_function != '':
                if r.aggregate_operators != '' and len(r.aggregate_value) > 0:
                    df = dm.group_by_having(df, r.group_by, r.aggregate_function, r.aggregate_operators,
                                            r.aggregate_value)
                else:  # if just grouping and aggregate function is specified
                    df = dm.group_by_having(df, r.group_by, r.aggregate_function)
            else:  # if just grouping is specified
